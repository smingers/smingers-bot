...
**Why the ensemble was off:** We predicted 80% that the Metaculus community prediction (CP) for “Will Netanyahu cease to be PM during 2026?” would be above 30% on 2026-02-11. The outcome was **No**—CP did not exceed 30%. So we were badly wrong on both direction and magnitude. Top performers gave roughly 3% (empirical same-question base rate) and ~15% (volatility model with negative drift); they treated a +4pp reversal as unlikely. We never knew the current CP was 26% or that it had just dropped from 33–37%; we inferred a baseline of 35–48% from Manifold and concluded we were already above 30% with room to spare. One missing input—the actual Metaculus time series—drove the entire error.

**Evidence we overweighted**

- **Manifold as a stand-in for Metaculus.** We used Manifold’s 39–44% for “Netanyahu leaves in 2026” as the main anchor and assumed Metaculus would be similar or within a few points. We derived “current Metaculus” in the 31–48% range and thus treated the 30% threshold as already cleared or easily cleared. Manifold and Metaculus can diverge; on this question Metaculus CP was **26%**, not 39%. We never had the target platform’s number, so we never saw that we needed a **+4pp upward move** from a recent drop.
- **Israeli political fundamentals as support for “CP stays high.”** We weighted budget deadline (March 31), coalition instability, the 53% “Netanyahu should go” poll, and the Supreme Court case as reasons CP would stay above 30%. The question was whether **CP would be >30% on Feb 11**, i.e. short-horizon prediction-market dynamics. Top performers noted that the main structural deadlines (budget, court hearing) lay **outside** the Feb 4–11 window, so the next week had no obvious catalyst—and recent news had been stabilizing, consistent with CP having **fallen** from 37% to 26%. We treated “reasons Netanyahu might eventually leave” as reasons CP would stay high, without checking whether CP had already moved down or whether any catalyst would hit in the next 7 days.
- **Generic reference classes instead of same-question history.** We used cross-question stats (e.g. “38 questions, 72% on modal side,” “σ≈4pp weekly move”) and Manifold-based volatility. The 3% forecaster used the **same question’s** 46-day history and 339 CP measurements: in the **recent stable regime** (Jan 21–Feb 4, after ACX scoring cutoff), there were **0 instances of a +4pp upward move** in 22,017 seven-day windows (0% base rate). We had no time series, so we couldn’t condition on “this question, post-cutoff, low-volatility regime” and instead assumed a generic “CP near Manifold with normal volatility,” which was the wrong regime.

**Evidence we underweighted or missed**

- **Actual Metaculus CP and its recent path.** The assessment already flags this: “The actual Metaculus community prediction was never retrieved.” For a meta-question about a specific Metaculus CP at a specific time, the primary input is that question’s **current level and recent trajectory**. We never fetched it. Top performers had: current CP 26%, “last week” 33–37%, so a **−7 to −11pp drop** in a week. That told them the burden of proof was on a **reversal**, not on “staying high.” We never had that burden.
- **Regime change after ACX cutoff.** The 3% forecaster noted the ACX contest scoring cutoff (Jan 17) and that the question had since entered a low-volatility, consensus regime (CP range 26–37% in the stable period vs 25–60% in the early volatile period). We had no time series, so we couldn’t identify regime or use a regime-specific base rate (0% for +4pp up in stable period).
- **“No catalyst in window” as a reason to be cautious.** twsummerbot framed the week as “short-term forecasting dynamics (forecaster attention, mean reversion, or news shock)” with no scheduled deterministic event; they used slight **negative** drift (μ≈−1 to 0) and got P(CP>30) ≈ 13–16%. We assumed drift would keep CP near “baseline” (wrong baseline) and didn’t explicitly ask “what could move CP up 4pp in 7 days?” or “is there a scheduled event in this window?”

**Takeaways:** (1) For meta-questions about a **specific** platform’s prediction at a **specific** time, the first research step is to retrieve that platform’s **current value and time series** for the target question; do not substitute another platform (Manifold) without checking the target. (2) When the resolution is “CP above X at time T,” condition on the **current** CP and the **required move** (e.g. +4pp reversal); use same-question history and regime (e.g. post-cutoff, stable) for base rates when available. (3) Separate “reasons the underlying event might happen” from “reasons the CP will move in the next N days”; lack of catalysts in the window should push toward stability or continuation of recent trend, not toward our prior from a different data source.
